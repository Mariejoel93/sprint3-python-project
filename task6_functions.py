...
# a) Function to calculate total NA + EU + JP sales for one game
def calculate_total_sales(game):
    return game[NA_SALES] + game[EU_SALES] + game[JP_SALES]

# Test it on the first game
print(calculate_total_sales(video_game_sales[0]))

# b) Function to filter games by genre (default genre is 'Platform')
...
# c) Function to build a one-line summary string for a game
def get_summary(game):
    return f"{game[NAME]} ({game[YEAR]}) - {game[GENRE]} - ${game[GLOBAL_SALES]}M"

# Use it in a loop to print a summary of every game
